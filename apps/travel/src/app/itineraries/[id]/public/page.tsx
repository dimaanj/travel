...
import { getItinerariesList, getItineraryById } from '../../../../lib/server-data';
...
export async function generateStaticParams() {
  ...
  return itineraries.map((itinerary) => ({ id: itinerary.id }));
}
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    ...
    description: itinerary.description ?? `Travel itinerary: ${itinerary.title}`,
  };
}
...
export default async function ItineraryPublicPage({ params }: PageProps) {
  const { id } = await params;
  const itinerary = await getItineraryById(id);
